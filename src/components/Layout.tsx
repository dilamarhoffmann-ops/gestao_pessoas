...
import { LayoutDashboard, Users, Scale, DollarSign, Activity, Settings, ChevronLeft, Menu } from 'lucide-react';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-bg-main text-deep-navy font-sans selection:bg-primary/30 overflow-hidden">
      ...
      <aside className={cn(
        "bg-sidebar border-r border-white/5 flex flex-col py-8 z-30 shadow-2xl transition-all duration-300",
        ...
        <nav className="flex-1 flex flex-col gap-4 w-full">
          <SideNavItem to="/" icon={<LayoutDashboard size={20} />} label="Início" isExpanded={isExpanded} />
          <SideNavItem to="/discounts" icon={<DollarSign size={20} />} label="Financeiro" isExpanded={isExpanded} />
          <SideNavItem to="/lawsuits" icon={<Scale size={20} />} label="Jurídico" isExpanded={isExpanded} />
          <SideNavItem to="/hiring" icon={<Users size={20} />} label="Gestão" isExpanded={isExpanded} />
        </nav>
        ...
        <div className={cn(
          ...
          <button className={cn(
            "text-white/40 hover:text-white transition-colors flex items-center gap-4 px-2 py-2 rounded-lg hover:bg-white/5 w-full",
            !isExpanded && "justify-center"
          )}>
            <Settings size={20} />
          ...
          <div className={cn(
            "flex items-center gap-3 w-full p-2 bg-white/5 rounded-xl border border-white/5",
            !isExpanded && "justify-center"
          )}>
            <div className="w-8 h-8 rounded-lg bg-primary/20 border border-white/10 flex items-center justify-center text-[10px] font-bold text-white shrink-0">
              AD
            </div>
            {isExpanded && (
              <div className="flex flex-col min-w-0">
                <span className="text-[10px] font-bold text-white truncate">Audit Admin</span>
                <span className="text-[8px] font-medium text-white/40 uppercase tracking-widest">Premium Plan</span>
              </div>
            )}
          </div>
        </div>
